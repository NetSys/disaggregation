...
def schedule(flows, waiting_flow_ids, running_flow_ids, ibusy, ebusy, time, events, events_time, host_delay):
    ...
    max_match, _, _ = hka.bipartiteMatch(graph)

    for dest, src in max_match.iteritems():
        flow_id = min(flow_id_map[(src, dest)], key = lambda x: flows[x]['remaining size'])
        flow = flows[flow_id]
...
def swordfish_a(flows, host_delay, pod_size=16, pod_num=9):
    '''
    Args:
...
def get_swordfish_a_fcts(lines, host_delay):
    ...
    swordfish_a(flows, host_delay)
    swordfish_a_fcts = [flows[i]['ideal fct'] for i in range(len(flows))]
    return swordfish_a_fcts


if __name__ == '__main__':
    get_swordfish_a_fcts(open('reproduce_figure8/Dataset/flow_0.2Load.tr').readlines(), 2.5)
