...
{
  queue = Factory::get_queue(id, rate, params.queue_size, type);
}
  ...
  : Switch(id, CORE_SWITCH)
{
  for (uint32_t i = 0; i < nq; i++) {
    queues.push_back(Factory::get_queue(i, rate, params.queue_size, type));
  }
}
...
AggSwitch::AggSwitch(uint32_t id, uint32_t nq1, double r1,
  ...
  for (uint32_t i = 0; i < nq1; i++) {
    queues.push_back(Factory::get_queue(i, r1, params.queue_size, type));
  }
  for (uint32_t i = 0; i < nq2; i++) {
    queues.push_back(Factory::get_queue(i, r2, params.queue_size, type));
  }
}
