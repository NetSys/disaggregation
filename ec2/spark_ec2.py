...
def get_existing_cluster(conn, opts, cluster_name, die_on_error=True, silent = False):
    if not silent:
...
def mosh_command(opts):
    return ['mosh',  '--ssh=\"ssh ' + " ".join(ssh_args(opts)) + '\"']
...
def real_main():
    ...
    if action == "launch":
        ...
        master = master_nodes[0].public_dns_name
        print "Logging into master " + master + "..."
        cmd = "mosh --ssh=\"ssh -i %s\" %s@%s" % (opts.identity_file ,opts.user, master)
        print cmd
        os.system(cmd)
        # proxy_opt = []
        # if opts.proxy_port is not None:
        #     proxy_opt = ['-D', opts.proxy_port]
        # cmd = mosh_command(opts) + proxy_opt + ["%s@%s" % (opts.user, master)]
        # print cmd
        # subprocess.check_call(cmd)
    elif action == "install-mosh":
        (master_nodes, slave_nodes) = get_existing_cluster(conn, opts, cluster_name)
        master = master_nodes[0].public_dns_name
        print "Logging into master " + master + "..."
        cmd = "ssh -i %s %s@%s \"yum --enablerepo=epel install -y mosh\"" % (opts.identity_file ,opts.user, master)
        print cmd
        os.system(cmd)
        ...
        (master_nodes, slave_nodes) = get_existing_cluster(conn, opts, cluster_name, silent = True)
