...
def schedule(flows, waiting_flows_id, waiting_flow_remain_size, ibusy, ebusy, time, events, events_time, host_delay, cut_through):
    ...
    for flow_id in waiting_flows_id_copy: # waiting_flows_id is sorted by remaining size
        ...
        if flow['state'] == 'created' or flow['state'] == 'suspended':
            if not ibusy[iport][0] and not ebusy[eport][0]:
                ...
                remaining_size1 = flow1['remaining size'] if flow1 else float('Inf')
                remaining_size2 = flow2['remaining size'] if flow2 else float('Inf')
                if flow['remaining size'] < remaining_size1 and flow['remaining size'] < remaining_size2:
                    run = True
                    def suspend(flow, waiting_flows_id, waiting_flow_remain_size, time, events, events_time):
                        if flow:
                            ...
                            waiting_flow_remain_size.insert(index, flow['remaining size'])
                            waiting_flows_id.insert(index, flow['id'])
                    suspend(flow1, waiting_flows_id, waiting_flow_remain_size, time, events, events_time)
                    if flow2 != flow1:
                        suspend(flow2, waiting_flows_id, waiting_flow_remain_size, time, events, events_time)
            ...
            if run:
                ...
                del waiting_flow_remain_size[index]
...
def ideal(flows, host_delay, cut_through, pod_size=16, pod_num=9):
    ...
    while events:
        ...
        if e['type'] == 'create': #Flow created
            ...
            index = bisect.bisect_left(waiting_flow_remain_size, flows[flow_id]['remaining size'])
            waiting_flow_remain_size.insert(index, flows[flow_id]['remaining size'])
...
def get_ideal_fcts(lines, host_delay, cut_through=False, bandwidht=10000):
    # Bandwidth given in Mbps
    ...
    flows = {
                int(lines[i].split()[0]): {
                    ...
                    'hop delay': 0.2, # in us
                    'low bandwidth': bandwidth, # bandwidth in bits/us
                    'high bandwidth': 4 * bandwidth, # bandwidth in bits/us
                }
    ...
    ideal_fcts = [(i, int(flows[i]['size']/8), flows[i]['src'], flows[i]['dest'], flows[i]['create time'], flows[i]['finish time'], flows[i]['ideal fct'], flows[i]['oracle fct'], flows[i]['ideal fct']/flows[i]['oracle fct']) for i in range(len(flows))]
