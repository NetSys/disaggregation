...
void FlowFinishedEvent::process_event() {
	std::cout << "FlowFinishedEvent::process_event():" << flow->id << " " << flow->size << " " << flow->src->id << " "
			...
			<< 1000000 * flow->flow_completion_time
					/ topology->get_oracle_fct(flow) << std::endl;
}
...
void PacketQueuingEvent::process_event() {
	if (!queue->busy) {
		queue->queue_proc_event = new QueueProcessingEvent(get_current_time(),
				queue);
	...
	if (packet) {
		...
		if (next_hop == NULL) {
			...
			if (params.cut_through == 1) {
				double cut_through_delay = queue->get_transmission_delay(
						packet->flow->hdr_size);
				add_to_event_queue(
						new PacketQueuingEvent(time + cut_through_delay + pd,
								packet, next_hop));
			} else {
				add_to_event_queue(
						new PacketQueuingEvent(time + td + pd, packet,
								next_hop));
			}
		}
	} else {
...
void FlowCreationForInitializationEvent::process_event() {
	...
	std::cout << 1000000.0 * time << " Generating new flow " << id
			<< " of size " << size << " between " << src->id << " " << dst->id
			<< "\n";

	double tnext = time + nv_intarr->value();
	add_to_event_queue(
			new FlowCreationForInitializationEvent(tnext, src, dst, nv_bytes,
					nv_intarr));
}
...
void FlowCreationForInitializationEventWithTimeLimit::process_event() {
	...
	if (tnext < time_limit)
		add_to_event_queue(
				new FlowCreationForInitializationEventWithTimeLimit(time_limit,
						tnext, src, dst, nv_bytes, nv_intarr));
}

LoggingEvent::LoggingEvent(double time) :
		Event(LOGGING, time) {
}
